...
async function checkJsonStructure() {
  ...
  try {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (Array.isArray(data) && data.length > 0) {
            ...
            if (data[0]) {
              const requiredFields = ['id', 'name', 'type', 'status', 'value', 'temperature'];
              const missingFields = requiredFields.filter(field => !(field in data[0]));
              
              if (missingFields.length > 0) {
                console.warn(`⚠️ Отсутствуют поля: ${missingFields.join(', ')}`);
                console.log('Доступные поля:', Object.keys(data[0]));
              }
            }
          }
        } else {
          console.log(`❌ Не удалось получить данные с ${endpoint}: ${response.status}`);
        }
      } catch (err) {
    ...
    if (window.TARGET_WS) {
      console.log('\n--- Проверка WebSocket ---');
      console.log(`WebSocket URL: ${window.TARGET_WS}`);
    } else {
      console.log('\n⚠️ TARGET_WS не настроен');
    }
...
function analyzeStructure(data, endpoint) {
  if (Array.isArray(data)) {
    ...
    if (data.length > 0) {
      ...
      const deviceTypes = [...new Set(data.map(item => item.type || item.Type || 'неизвестно'))];
      console.log(`Типы устройств: ${deviceTypes.join(', ')}`);
      
      // Проверяем статусы
      const statuses = [...new Set(data.map(item => item.status || item.Status || 'неизвестно'))];
