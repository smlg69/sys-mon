...
const isDoubleEncodedCp1251 = (text: string): boolean => {
  return /РЎ|Рў|Р Р|Р›|Рњ|Рќ/.test(text) ||
         /СЂ|С‚|С |С›|Сњ|Сќ/.test(text);
};
...
export const requestsApi = {
    ...
    try {
      ...
      const decodedOrders = response.map((order: any) => {
        ...
        Object.keys(order).forEach(key => {
          ...
          if (key === 'id' && (typeof value === 'number' || /^#\d+$/.test(value))) {
            decoded[key] = value;
          } else {
  ...
  updateOrderStatus: async (orderId: string | number, status: string): Promise<any> => {
    try {
      ...
      // Используем обычный POST, так как сервер может не принимать withDecoding
      return await apiClient.post(`${FUNCTIONS}'/updateOrderF', [dataToSend]`);
    } catch (error) {
  ...
  updateOrder: async (orderId: string | number, updateData: {
    ...
    try {
      ...
      if (updateData.user) dataToSend.nUser = updateData.user;
      ...
      return await apiClient.post('/updateOrderF', [dataToSend]);
    } catch (error) {
  ...
  getUsers: async (): Promise<User[]> => {
    try {
      ...
      const users: User[] = response.map((user: any) => ({
        ...
        name: decodeText(user.name || user.full_name || user.username || 'Неизвестный'),
  ...
  assignOrder: async (orderId: string | number, userId: string, userName: string): Promise<any> => {
    try {
      ...
      const response = await apiClient.post('/updateOrderF', [dataToSend]);
  ...
  testUpdateFormat: async (orderId: string | number, userName: string): Promise<any> => {
    ...
    const formats = [
      { data: [{ id: orderId.toString(), nUser: userName }], name: 'Только nUser' },
      { data: [{ id: orderId.toString(), nUser: userName, status: 'В работе' }], name: 'nUser + статус' },
      { data: { id: orderId.toString(), nUser: userName }, name: 'Объект вместо массива' },
    ];
