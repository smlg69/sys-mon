...
export const fetchReportData = async (
  ...
  try {
    ...
    // Определяем endpoint
    const endpoints = {
      orders: '/rest/v1/contexts/users.admin.models.workerMS/functions/reportForOrdersXlsxF',
      devices: '/rest/v1/contexts/users.admin.models.workerMS/functions/reportForDevicesXlsxF',
      kpi: '/rest/v1/contexts/users.admin.models.workerMS/functions/reportForKPIXlsxF'
    };
...
const parseExcelFromBase64Simple = async (base64String: string, reportType: string): Promise<any[]> => {
  try {
    ...
    console.log(`📊 Диапазон: строк ${range.e.r + 1}, колонок ${range.e.c + 1}`);
    ...
    for (let r = 0; r < Math.min(10, range.e.r + 1); r++) {
      ...
      if (rowText.includes('id') && (rowText.includes('type') || rowText.includes('device') || rowText.includes('status'))) {
        headerRow = r;
        foundHeaders = true;
    ...
    const filteredData = jsonData.filter((item: any) => {
      ...
      if (firstValue && typeof firstValue === 'string') {
        ...
        if (firstValue.includes('Отчет по') || 
  ...
  try {
    ...
    const createHtmlContent = () => {
      ...
      const tableHeaders = columns.map(col => 
        `<th>${headerTranslations[col] || col}</th>`
      ).join('');
      
      // Создаем строки таблицы (ограничиваем 100 записями для производительности)
      const tableRows = reportData.slice(0, 100).map((item: any) => {
    ...
    if (!printWindow) {
      throw new Error('Не удалось открыть окно для печати. Пожалуйста, разрешите всплывающие окна в браузере.');
    }
...
export const generatePdfReport = async ({
  ...
  try {
    ...
    if (err.message.includes('Не удалось открыть окно')) {
      errorMessage = 'Браузер заблокировал всплывающее окно. Разрешите всплывающие окна для этого сайта и попробуйте снова.';
    } else if (err.message.includes('Нет данных')) {
      errorMessage = 'Нет данных для формирования отчета. Проверьте параметры отчета.';
