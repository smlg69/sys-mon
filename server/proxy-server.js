...
console.log(`🎯 Target WebSocket: ${TARGET_WS}`);
...
wss.on('connection', (clientWs, request) => {
  ...
  let targetUrl = TARGET_WS;
  if (token) {
    targetUrl += (TARGET_WS.includes('?') ? '&' : '?') + `token=${encodeURIComponent(token)}`;
  }
  ...
  clientWs.on('message', (message) => {
    if (targetWs.readyState === WebSocket.OPEN) {
      try {
        ...
        console.log(`   📤 [${connectionId}] Client -> Backend (${msgStr.length} chars)`);
        targetWs.send(msgStr);
      } catch (error) {
        console.error(`   ❌ [${connectionId}] Send error:`, error.message);
      }
    } else {
      console.log(`   ⚠️ [${connectionId}] Can't forward, backend not connected`);
    }
  });
  ...
  targetWs.on('message', (message) => {
    if (clientWs.readyState === WebSocket.OPEN) {
      try {
        const msgStr = message.toString();
        console.log(`   📥 [${connectionId}] Backend -> Client (${msgStr.length} chars)`);
  ...
  targetWs.on('error', (error) => {
    clearTimeout(connectTimeout);
    console.error(`   ❌ [${connectionId}] Backend connection error:`, error.message);
  ...
  targetWs.on('close', (code, reason) => {
    clearTimeout(connectTimeout);
    console.log(`   🔌 [${connectionId}] Backend closed: ${code} ${reason || ''}`);
  ...
  clientWs.on('close', (code, reason) => {
    clearTimeout(connectTimeout);
    console.log(`   🔌 [${connectionId}] Client closed: ${code} ${reason || ''}`);
...
server.listen(PROXY_PORT, '0.0.0.0', () => {
  ...
  console.log(`   🎯 Forwarding to: ${TARGET_WS}`);
